...
def convert_dict(dictionary: dict[str, _T]) -> dict[str, _T]:
    ...
    return {
        (convert_key(k) if isinstance(k, str) else k): (
            convert_dict(v) if isinstance(v, dict) else v  # type: ignore[misc]
        )
        for k, v in dictionary.items()
    }


def convert_until(status_dict: dict[str, Any], until_key: str) -> None:
...
def dt_aware_to_naive(dt_aware: datetime) -> datetime:
    """Convert a TZ-aware datetime to a local/naive datetime."""
...
def dt_local_to_aware(dt_naive: datetime) -> datetime:
    """Convert a local/naive datetime to a TZ-aware datetime."""
